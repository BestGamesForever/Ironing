...
namespace ScratchCardAsset.Core
{
	...
	public class ScratchCardInput
	{
		...
		public delegate Vector2 ScratchHandler();
	...
	private bool isWebgl = true;
		...
		public void Update()
		{
			Debug.Log("Hello"+GameObject.FindGameObjectWithTag("Player").transform.localPosition);
			...
			else
			{
				...
				{
					TryScratch(0, Input.mousePosition);//GameObject.FindGameObjectWithTag("Player").transform.localPosition);
				}
		...
		private void TryScratch(int fingerId, Vector3 position)
		{
			if (OnScratch != null)
			{
				erasePosition = OnScratch();
			}
