...
namespace ScratchCardAsset
{
	public class ScratchCard : MonoBehaviour
	{
		...
		private Vector2 GetScratchPosition()
		{
			...
			{
				float planeY = 0;
				var plane = new Plane(Vector3.up,Vector3.up* planeY);
				var ray = new Ray(Iron.transform.position+new Vector3(0,1,0), Vector3.down);//MainCamera.ScreenPointToRay(position);
				Debug.DrawRay(Iron.transform.position + new Vector3(0, 1, 0), Vector3.down, Color.blue, 1);
				float enter;
